	...
	column(
		...
		if (start !== 0 || end !== container.children.length) {
			...
			if (!range) {
				...
				range.setToolActivityVisible(!displayPreferences.hideToolActivity);
...
export class RewindHistory {
	...
	/** Full rendered search is explicit; ordinary navigation never visits every chunk. */
	items(width: number): RewindHistoryItem[] {
		const items: RewindHistoryItem[] = [];
		for (let chunk = 0; chunk < this.#chunks.length; chunk++) {
			const source = this.#chunks[chunk]!;
			const childRows = this.#searchRows.rows(source.builder.container.children, Math.max(10, width - 4));
			for (let target = 0; target < source.targets.length; target++) {
				const outline = source.targets[target]!;
				const rows = childRows.slice(outline.start, outline.end).flat();
				if (!rows.some(row => /\S/.test(Bun.stripANSI(row)))) continue;
				items.push({ point: { chunk, target }, target: outline, rows });
			}
		}
		return items;
	}

	/** Select a point returned by this history's search without replaying it again. */
	select(point: RewindPoint): void {
		this.#prepared = true;
		this.#selected = point;
	}

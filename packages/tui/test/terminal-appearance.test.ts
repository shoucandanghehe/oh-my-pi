...
describe("ProcessTerminal OSC 11 appearance detection", () => {
	...
	it("periodically re-queries OSC 11 under native Windows Terminal when Mode 2031 is unavailable (#5091)", () => {
		...
		// DEC 2048, DEC 2031, DEC 1016, DEC 2004, and xterm ?1010/?1011.
	...
	it("refreshAppearance() issues exactly one OSC 11 re-query per call (#5352)", () => {
		...
		// OSC 11, and the DECRQM probes for 2026/2048/2031/1016/2004/1010/1011)
	...
	it("kitty keyboard probe owns its own DA1 sentinel — does not consume OSC 11's", () => {
		...
		// Nine DA1 sentinels are in flight at startup: keyboard probe, OSC 11,
		// and the DECRQM probes for DEC 2026, 2048, 2031, 1016, 2004, 1010, and
		// 1011 (each rides the shared FIFO). Consume them in send order and verify
		// none leaks to the input handler.
		for (let i = 0; i < STARTUP_DA1_SENTINELS; i++) {
			process.stdin.emit("data", "\x1b[?1;2c");
		}
		expect(received).toEqual([]);
